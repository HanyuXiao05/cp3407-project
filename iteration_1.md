...
* Secure Online Registration, priority High, 6 (initial: 2) days  
   - Analyse form considerations (user roles, validation, compliance) (MTN), 22 June 2025
   - Backend registration logic (hashing, storage, CSRF protection) (YJ), 2 days 
   - Backend registration logic (email) (HX), 1 day
   - Frontend form validation + accessibility (TQRN), 1 day 
   - Testing for registration flow (All), 1 day 
* Membership Payment Handling, priority Low, 4 (initial: 3) days
   - Backend online payment integration (mock transaction logic, success/failure states, storage) (YJ), 1 day
   - Backend offline payment integration (storage) (HX), 0.5 day
   ...
   - Backend membership activation logic (storage, status) (HX), 0.5 day
   - Frontend membership confirmation (status) (TQRN), 0.5 day
   - Testing for membership handling flow (All), 1 day 
* Session Booking, priority High, 5 (initial: 3) days
   - Backend booking logic (slot availability, booking action + status, storage) (HX,YJ), 1 day
   - Frontend display (calendar, slot availability, booking action + status) (MTN), 1 day
   - Backend booking logic (confirmation email (DRY), reminder emails) (YJ), 1 day
   - Integrated user authentication for session booking (YJ),1 day
   - Testing for booking flow (All), 1 day
